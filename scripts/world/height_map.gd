...
func generate(
	...
	p_falloff_noise_scale: float = 60.0,
	p_falloff_noise_amount: float = 0.0,
) -> void:
	width = p_width
	height = p_height
	noise_values = _build_noise(p_seed, p_scale, p_octaves, p_persistence, p_lacunarity, p_offset, p_normalize_mode)
	falloff_values = _build_falloff(p_falloff_shape, p_seed, p_falloff_noise_scale, p_falloff_noise_amount)
...
## nx/ny distance from centre, perturbed by its own noise field before the
## S-curve — an island coastline instead of a straight square/radial edge.
## amount 0 reproduces the un-perturbed distance exactly.
func _build_falloff(shape: FalloffShape, p_seed: int, noise_scale: float, noise_amount: float) -> PackedFloat32Array:
	var coast_noise: FastNoiseLite = FastNoiseLite.new()
	coast_noise.noise_type = FastNoiseLite.TYPE_PERLIN
	coast_noise.seed = p_seed + 1
	coast_noise.frequency = 1.0
	var scale: float = noise_scale if noise_scale > 0.0 else 0.0001
	...
	for y in range(height):
		for x in range(width):
			...
			var perturb: float = coast_noise.get_noise_2d(x / scale, y / scale) * noise_amount
			result[y * width + x] = _evaluate_falloff(clampf(value + perturb, 0.0, 1.0))
	return result
