...
const HUMAN_PATH := "res://scripts/entities-and-species/species/human.tres"
const DRYAD_PATH := "res://scripts/entities-and-species/species/dryad.tres"
...
static func _settle(
	...
	for index in population:
		...
		entity.remember(_home_memory(species, centre, entity.born_at), entity.born_at)
...
## What every founder already knows: where home is.
static func _home_memory(species: Species, centre: Vector3, formed_at: float) -> Memory:
	var memory: Memory = Memory.new()
	memory.what = species.settlement_name
	memory.position = centre
	memory.when = formed_at
	memory.is_place = true
	return memory
