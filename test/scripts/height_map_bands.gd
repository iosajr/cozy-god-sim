...
## Terrace level for a value. Every band below the top is one level, same as
## index_for — but the top band keeps climbing past its own threshold
## instead of capping, so a mountain peak stacks up to max_peak_levels extra
## steps above it rather than sitting on one flat plateau.
static func level_for(value: float, bands: Array[Dictionary], max_peak_levels: int) -> int:
	var band_index: int = index_for(value, bands)
	if band_index < bands.size() - 1:
		return band_index
	var peak_t: float = inverse_lerp(bands[band_index]["threshold"], 1.0, value)
	return band_index + int(round(peak_t * max_peak_levels))
