...
func test_query_next_task_returns_a_real_idle_task_when_nothing_is_urgent() -> void:
	# issue #29: revises the old "returns null when nothing urgent
	# applies" behavior — a real, lowest-priority Task.KIND_IDLE is
	# always handed out instead.
	var village := Village.new()
	var villager := Villager.new("v1", true, "The bread smells almost ready.")
	...
	assert_eq(task.priority, Village.IDLE_PRIORITY)
	assert_false(task.is_must_do())


func test_idle_priority_is_below_the_lowest_eat_and_sleep_priorities() -> void:
	# issue #29's Implementation Decisions: IDLE_PRIORITY must sit below
	# EAT_PRIORITY_HUNGRY/SLEEP_PRIORITY_TIRED (the lowest currently-
	# assigned real-need priorities) so Idle never outranks a real need.
	assert_true(Village.IDLE_PRIORITY < Village.EAT_PRIORITY_HUNGRY)
	assert_true(Village.IDLE_PRIORITY < Village.SLEEP_PRIORITY_TIRED)
...
func test_should_interrupt_is_true_when_current_is_idle_and_candidate_is_a_non_must_do_real_task() -> void:
	# issue #29's User Story 3: Idle is preempted by any real (non-Idle)
	# candidate immediately, not just a Must-do one — wandering has no
	# near-finished/consequential cost worth protecting.
	var village := Village.new()
	var current := Task.new(Task.KIND_IDLE, Village.IDLE_PRIORITY)
	var candidate := Task.new(Task.KIND_EAT, Village.EAT_PRIORITY_HUNGRY)
...
func test_should_interrupt_is_false_when_current_and_candidate_are_both_idle() -> void:
	# query_next_task() constructs a fresh Task instance every call —
	# treating a same-kind Idle candidate as a real replacement would
	# re-interrupt (and reset) the wander loop every single frame nothing
	# has actually changed.
	var village := Village.new()
	var current := Task.new(Task.KIND_IDLE, Village.IDLE_PRIORITY)
	var candidate := Task.new(Task.KIND_IDLE, Village.IDLE_PRIORITY)
...
func test_advance_task_assignment_assigns_an_idle_task_to_a_free_villager_with_nothing_urgent() -> void:
	# issue #29: revises the old "leaves a free villager untasked"
	# behavior — a free Villager with nothing urgent now gets a real
	# Idle Task instead of staying at null.
	var village := Village.new()
	var villager := Villager.new("v1", true, "The bread smells almost ready.")
...
func test_advance_task_assignment_does_not_thrash_a_running_idle_task_every_frame() -> void:
	# Regression for issue #29's should_interrupt() Idle exception: even
	# though query_next_task() re-queries every call, without the
	# Idle-vs-Idle guard in should_interrupt() this would re-"interrupt"
	# (and reset the wander state of) an already-idle Villager every
	# single frame.
	var village := Village.new()
	var villager := Villager.new("v1", true, "The bread smells almost ready.")
...
func test_query_next_task_reuses_a_single_idle_task_instance_across_calls_and_villagers() -> void:
	# query_next_task() caches one shared Task.KIND_IDLE instance rather
	# than allocating a fresh one on every call (issue #29's code review)
	# — Task is immutable plain data once constructed, and all of Idle's
	# real per-Villager state lives in Village's own dictionaries keyed
	# by Villager, never on the Task instance, so sharing one instance
	# across every currently-idle Villager is safe.
	var village := Village.new()
...
func test_advance_task_assignment_switches_from_idle_to_a_non_must_do_real_task_immediately() -> void:
	# issue #29's User Story 3: even a merely-Important (non-Must-do) Eat
	# candidate immediately preempts a running Idle Task.
	var village := Village.new()
	var villager := Villager.new("v1", true, "The bread smells almost ready.")
	village.advance_task_assignment(villager)
	assert_eq(villager.current_task.kind, Task.KIND_IDLE)
...
# --- advance_farms() (issue #15) ---
# Mirrors advance_eating_checks()'s existing test shape (countdown
# ticking, not-yet-elapsed vs. elapsed) per issue #15's Testing
# Decisions.
...
func test_idle_destination_stays_within_the_wander_radius_of_site_position() -> void:
	...
	assert_true(destination.distance_to(village.site_position) <= Village.IDLE_WANDER_RADIUS)
...
func test_idle_destination_is_tracked_per_villager() -> void:
	...
	# Both land within radius of the same site_position, but nothing
	# forces them to share a dict entry — a smoke check that per-Villager
	# tracking doesn't cross-contaminate.
	assert_true(village.idle_destination(villager_a).distance_to(village.site_position) <= Village.IDLE_WANDER_RADIUS)
	assert_true(village.idle_destination(villager_b).distance_to(village.site_position) <= Village.IDLE_WANDER_RADIUS)
...
func test_advance_idle_does_not_pick_a_new_point_before_the_stand_duration_elapses() -> void:
	var village := Village.new()
	var villager := Villager.new("v1", true, "The bread smells almost ready.")
	villager.current_task = Task.new(Task.KIND_IDLE, Village.IDLE_PRIORITY)
	village.begin_resolving_task(villager, {"food": 100}, 1.0)
...
func test_advance_idle_picks_a_new_point_and_resumes_traveling_once_the_stand_duration_elapses() -> void:
	...
	# IDLE_STAND_SECONDS_MAX is the longest a stand phase can last —
	# advancing past it guarantees the phase has ended regardless of
	# which random duration was actually rolled.
	var started_new_leg := village.advance_idle(villager, Village.IDLE_STAND_SECONDS_MAX + 1.0)
...
func test_advance_idle_is_a_noop_while_still_traveling_not_yet_resolving() -> void:
	...
	villager.current_task = Task.new(Task.KIND_IDLE, Village.IDLE_PRIORITY)
	# Note: begin_resolving_task() was never called, so task_resolving
	# stays false — still traveling toward the current wander point.
...
func test_advance_idle_is_a_noop_for_a_villager_with_a_different_task_kind() -> void:
	...
	# Should not raise even though no Idle countdown was ever started for
	# this Villager.
...
func test_interrupt_task_clears_a_resolving_idle_tasks_wander_state() -> void:
	...
	assert_null(villager.current_task)
	assert_false(villager.task_resolving)
	# A fresh Idle Task afterward starts resolving cleanly, with no
	# leftover countdown from the interrupted one.
	villager.current_task = Task.new(Task.KIND_IDLE, Village.IDLE_PRIORITY)
	assert_false(villager.task_resolving)
